...
public class TACGenerator extends MiniJavaBaseVisitor<TACBlock> {
    ...
    public TACBlock visitExpBinOp(MiniJavaParser.ExpBinOpContext ctx) {
        ...
        if (op.equals("<=")) {
            // <= should short-circuit.
			String resJ = this.genreg();
			String resI = this.genreg();
			String regResult = this.genreg();
			String isTrue = this.genlab();
			...
			result.addAll(expr1);
			result.add(TACOp.mov(resI, expr1.getResult())); // evaluates x
			result.addAll(expr2);
			result.add(TACOp.mov(resJ, expr2.getResult())); // evaluate y
			result.add(TACOp.binop(regResult, resJ, resI, 5)); //regresults = y>x
			
			result.add(TACOp.jz(regResult,mid)); //zero to regResult,middle
			result.add(TACOp.jmp(isTrue));
			result.add(TACOp.label(mid));
			result.add(TACOp.binop(t2, resI, resJ, 2)); 
			result.add(TACOp.jz(t2, isTrue));
			result.add(TACOp.jmp(isFalse));
			result.add(TACOp.label(isFalse));
			result.add(TACOp.immed(regResult, 0));
			
			result.add(TACOp.jmp(end));
			result.add(TACOp.label(isTrue));
			result.add(TACOp.immed(regResult,1));
			result.add(TACOp.label(end));
			
			result.setResult(regResult);
            return result;
        }
